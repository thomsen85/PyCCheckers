        ...
        for row in range(self.rows):
            for col in range(self.cols):
                if board[row][col] != 0:
                    ...
                    shape = arcade.create_ellipse(
                        x, y, self.radius, self.radius, Grid.COLOR
                    )
...
class GUI:
    ...
    def draw(self):
        ...
        if not self.selected_piece is None:
            ...
            move_coords = [
                self.grid.x_y_to_coord_x_y(m[0], m[1])
                for m in self.board.get_legal_moves(sel_x, sel_y)
            ]
            ...
            arcade.draw_circle_filled(
                sel_x, sel_y, self.grid.radius / 2, arcade.color.WHITE
            )
    ...
    def mouse_press(self, x, y):
        ...
        if self.selected_piece is None:
            ...
            print(legal_moves)
            if (
                self.board.get_board_value(x_coord, y_coord) == 1
                and (x_coord, y_coord) in legal_moves
            ):
